...
class SearchTopic:
    ...
    """A label for the type of diseases the searcher is diagnoising for"""

    possible_criteria: dict[str, str]
    """2-5 examples of affected body systems that can encapsulate what the patient is expereincing"""

@dc.dataclass
class DialogueForMultipleSearches:

    searcher: str
    """A label for the role of the person who needs help with an illness"""

    recommender: str
    """A label for the role of the person with the knowledge and resources to help with the diagnosis and provide recommendations and results"""
...
def update_corrected_schema(
    ...
    for schema_json in schemas_json:
        if domain is None: print(schema_json['diseases_type'])
        if schema_json['diseases_type'] != domain: continue
...
if __name__ == '__main__':
    ...
    generated_scenarios = generate_scenarios(10, save_folder='data/DOTS/train')
